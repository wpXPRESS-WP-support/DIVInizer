...
class ExpandDiviTwitterFeedWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		echo $args['before_widget'];
			...
			$statuses = get_transient( 'expand_divi_tweets_transient' );
			if ( ! $statuses ) {
				...
				foreach ( $statuses as $tweet ) {
				    ...
				    $item = '<div class="expand_divi_tweet_container">';
					$item .= '<p class="expand_divi_tweet_text">';
					$item .= $tweet_text;
					$item .= '</p>';
					$item .= '<p class="expand_divi_tweet_created_at"> - ';
					$item .= $formatted_date;
					$item .= '</p>';
				    $item .= '</div>';
				    echo $item;
				}
				set_transient( 'expand_divi_tweet_transient', $item, HOUR_IN_SECONDS );
			} else {
				echo $item;
			}
